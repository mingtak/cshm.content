    ...
    def __call__(self):
        ...
        if order:
            for item in order:
                ...
                orderList.append([status, apply_time, order_id, logistics_code, send_date])
...
class ManipulateMaterial(BrowserView):
    def __call__(self):
        ...
        execStr = """INSERT INTO `material`( `course`, `period`, `uid`, `status`, `send_date`, `logistics_code`, `detail`, `remark`,
                     `organizer`, `address`) VALUES('{}','{}','{}','{}','{}','{}','{}','{}', '{}', '{}')
                     """.format(course, period, uid, '處理中(寫死)', send_date, 'TODO', material_detail, remark, organizer, address)
        execSql.execSql(execStr)
        request.response.redirect(context_url + '/@@manipulate_material')
